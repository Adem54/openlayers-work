...
console.log("test: ",source.getFeatures())
...
//clusterSource.getFeatures() icersinde features lerden olusan VectorSource olmasina ragmen burasi [] bos array gelir 
//Ama direk clusterSource yi yazdirirsak icersinde features option in dizisini gorebiliriz
console.log("cluster... ",clusterSource);
//COOK ONEMLI BIR SEY BUUUUU BIZ CLUSTERSOURCE ICINE FEATURE EKLENMIS MI EKLENMEMIS MI BU SEKILDE ANLAYACAGIZ....BU COOOK ONEMLI...
const styleCache = {};
...
// const clusters = new VectorLayer({
  ...
  style: clusterStyle //1-Bu sekilde tamamen ayri bir fonksiyon icinde kullanabiliriz
});

//2-sonradan setStyle ile layer a yani clusters a da eklyebiliriz 
//clusters.setStyle(clusterStyle);
//Ve bu clusterStyle icerisinde biz normalde tek bir data olduklari zaman hangi style kullaniyorsak onun implemente edilmesini saglayabiliriz....

console.log("res: ",clusters.getSource())
...
//   console.log("features: ",feature.get("features"));//Burda cluster yani map te zoom-in yapinca fetaure ler orngin 12-4 ve 16-8 li 4 farkli gruba olustu ise o zaman burda bu fonksiyon 4 kez calisir ve 4 tane cluster var demektir... ve de her bir cluster(yani feature lerden olusan grup a dizi icinde olacak sekilde feature.get("features") bu sekilde erisiriz..  )
  ...
  // dynamic_text_color = selectedFeature.get("name") ? selectedFeature.get("name") :  dynamic_text;  
   style = styleForSingleFeature(selectedFeature) //BURDA HER BIR POINT ICIN UYGULANACAK OLAN STYLE I UYGULANIR AMA ASAGIDA SIZE E GORE VE YA CLSUTER GRUBUNA GORE STYLE AYARLAYABILIRIZ...BOYUTLARI DA YINE ONA GORE PLANLAYABILIRIZ... 
   }else if(size<=6){
     {
       if (!style) {
         style = new Style({
 ...
 let  dynamic_text_color = selectedFeature.get("name") ? selectedFeature.get("name") :  dynamic_text;  
  return [new Style({
    image: new CircleStyle({
      ...
      fill: new Fill({
        color: '#3399CC',
        color:"rgba(2, 240, 228,05)" //Bu zaten her bir point in tek basina iken olmasini istedgimiz style oldugu icin ona ozel color i direk veriyoruz
      }),
    }),
...
//BURASI HER BIR CLUSTER A TIKLANDIGINDA EGER CLUSTER LAR 1 DEN FAZLA ISE, ANIMASYONLU BIR SEKILDE TIKLANAN CLUSTER ICINDEKI FEATURE LARA YAKLASAK SEKILDE ONLARIN YERLERINI GOSTERIYOR... BURAYI KULLANMAZSAK CLUSTER I SADECE MOUSE ILE ZOOM-IN, ZOOM-OUT YAPINCA KULLANABILIRIZ AMA BURAYI KULLANARAK MOUSE TIKLAMASI ILE DE CLUSTER I KULLANABILMIS OLUYORUZ.. 
map.on('click', (e) => {
...
//Tum feature lara isimlerine veya icerisine verilen bilgye gore spesifik style atamasini bu sekilde de yapabiliyoruz... 
//Biz her zamn oncelikle elimzde var olan map,layer,source,features, geometry,vs vs gibi datalar uzerinden alt datalara erisip uygulamak istedigmz spesifk style vs ne varsa onu uyygulayabiliriz 
//Bu mantigin aynisi style a parametresine feature alan bir fonksiyon ile de yapabiliriz..Style ile yapilacak islemleri cok fazla kullanacagiz iyi hakim olmaliyiz
